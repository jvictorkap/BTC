...
df_renov_itau = pd.read_excel('G:\Trading\K11\Python\Aluguel\Tables\Kapitalo Renov.xlsx')
...
df_renovacao_tomadora=df_renovacao_tomadora.merge(df_cust_neg[['codigo','to_borrow_3','pos_doada']], on='codigo',how='inner')
df_renovacao_tomadora['to_borrow_3'].fillna(0,inplace= True)


df_renovacao_tomadora['trading_devol']=df_renovacao_tomadora.apply(lambda row: True if (row['to_borrow_3']!=0 and row['pos_doada']!=0) else False,axis=1)
...
df_renovacao_final= df_renovacao_tomadora[df_renovacao_tomadora['trading_devol']==False]
...
df_renovacao_final=df_renovacao_final.merge(df_renov_itau[['contrato','taxa final','Vencimento','Modalidade']],on='contrato', how= 'inner')


df_renovacao_final['tipo_registro']= df_renovacao_final['Modalidade'].apply(lambda x: 'N' if x=='D+1' else 'R')
df_renovacao_final['Modalidade']=df_renovacao_final['Modalidade'].apply(lambda x: 'E1' if x=='D+1'else 'E0' if x=='D0' else None)
...
boleta_renov=df_renovacao_final[['registro','cliente','corretora','tipo','vencimento','taxa','cotliq','reversor','codigo','contrato','saldo','Quantidade','taxa final','Vencimento','Troca?','tipo_registro','Modalidade','Tipo de Comissão','Valor fixo com']]
