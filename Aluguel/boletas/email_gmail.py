...
def save_attachments(message, directory, extensions, filename2save):
	...
	for part in message.walk():
		...
		if text is not None:
			...
			if '<html>' in text:
				try:
					value = text.split('R$')[1].split('<br>')[0].strip().replace('.', '').replace(',', '.')
		...
		if file_name:
			...
			if payload:
				...
				if fileNameCounter[file_name] > 1:
					new_file_name = '{file_name}({suffix}){ext}'.format(suffix=fileNameCounter[file_name],
				...
				try: 
					msgtime = dt.datetime.strptime(message.get_all('date', [])[0],"%a, %d %b %Y %H:%M:%S %z").replace(tzinfo=pytz.utc)
				except Exception:
					try:
						msgtime = dt.datetime.strptime(message.get_all('date', [])[0],"%d %b %Y %H:%M:%S %z").replace(tzinfo=pytz.utc)
					except Exception:
				...
				if filename2save !='':
					...
					if 'borrow' in file_name.lower():
						...
						new_file_name = f'{filename2save}_borrow_{msg_date}{file_extension}'
					elif 'loan' in file_name.lower():

						new_file_name = f'{filename2save}_loan_{msg_date}{file_extension}'
      
					else:
						new_file_name= f'{filename2save}_trade_{msg_date}{file_extension}'
      
				file_path = os.path.join(directory, new_file_name)
				print(file_path)
				if os.path.exists(file_path):
					filetime = dt.datetime.fromtimestamp(os.path.getmtime(file_path)).replace(tzinfo=pytz.utc)
					if filetime < msgtime:
						pass
					else:
						continue
...
def get_mail_files(senders, subject, str_local_directory, extensions, filename2save='', att_newer_than=24, str_search=None):
	if not str_search:
	...
	for msg in generate_mail_messages(user_name, password, resumeFile, str_search):
		...
		f_name, texts = save_attachments(msg, str_local_directory, extensions, filename2save)
...
def send_mail(mail_subject, mail_message_body, to_emails=['k11@kapitalo.com.br'], files=[]):
