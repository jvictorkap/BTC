...
def parse_excel_bofa(file_path):
    ...
    df=df[['Ticker','Quantity',	'Maturity',	'Rate'	,	'Callable'	,'Type','tipo_registro']]
    ...
    df['str_tipo']=df['Type'].apply(lambda x: 'D' if  x == 'Lender' else 'T' if x =='Borrower' else None)
    df['dbl_quantidade']=df.apply(lambda row: row['Quantity']*(-1) if row['str_tipo'] =='D' else row['Quantity'],axis=1)
    df.fillna(0,inplace=True)   
    df['str_tipo_registro']= df['tipo_registro'].apply(lambda x: 'R' if x=='Register' else 'N' if x=='T1' else None )
    df['str_modalidade']=df['str_tipo_registro'].apply(lambda x: 'D1' if x=='N' else None)
    ...
    df['dte_data']=workdays.workday(date.today(), 0, workdays.load_holidays()).strftime('%d/%m/%Y')
    df['dte_databoleta']=workdays.workday(date.today(), 0, workdays.load_holidays()).strftime('%d/%m/%Y')
