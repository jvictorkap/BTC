...
def parse_excel_mirae(file_path):
    ...
    df.rename(columns={'Ativo':'str_papel','Quantidade':'dbl_quantidade','Taxa': 'dbl_taxa'}, inplace=True)
    
    df['dte_datavencimento']=df['Vencimento'].apply(lambda x: datetime.date.strftime(x,'%Y-%m-%d'))
    df.fillna(0,inplace=True)
    df=df[df['str_papel']!=0]
    
    df["str_fundo"]= 'KAPITALO KAPPA MASTER FIM'
    df['str_corretora']="Mirae"   
    df['str_tipo_registro']=df['Modalidade'].apply(lambda x: 'R' if x== 'Balcão' else 'N' if x=='Eletrônico D+1' else None)
