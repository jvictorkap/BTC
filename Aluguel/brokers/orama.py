...
def parse_excel_orama(file_path):
    ...
    df.rename(columns={'PAPEL':'str_papel','QUANTIDADE':'dbl_quantidade','VCTO':'dte_datavencimento','TAXA':'taxa'}, inplace=True)
    ...
    df['str_tipo_registro']=df['MODALIDADE'].apply(lambda x: 'R' if x== 'Balcao' else 'N' if x=='Eletrônico D+1' else None)
...
def parse_excel_renov_orama(file_path):
    ...
    # df=df.columns(['MODALIDADE'	,'FUNDO'	,'CORRETORA'	,'PONTA'	,'VCTO',	'TAXA'	,'PAPEL'	,'QUANTIDADE'])
    df.rename(columns={'PAPEL':'str_papel','VCTO':'dte_datavencimento','TAXA':'taxa','PONTA':'str_tipo','CONTRATO':'contrato'}, inplace=True)
