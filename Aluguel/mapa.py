...
def main():
	...
	df_ctosaluguel_trade.rename(columns={'dbl_quantidade':'quantidade','str_papel':'codigo'}, inplace = True)

	df_doado_trade = df_ctosaluguel_trade[(df_ctosaluguel_trade['quantidade']<0) & (df_ctosaluguel_trade['ID']=='Emprestimo')]
	df_doado_trade = df_doado_trade.groupby('codigo', as_index=False).agg({'quantidade': sum})
	df_tomado_trade = df_ctosaluguel_trade[(df_ctosaluguel_trade['quantidade']>0) & (df_ctosaluguel_trade['ID']=='Emprestimo')]
	df_tomado_trade = df_tomado_trade.groupby('codigo', as_index=False).agg({'quantidade': sum})
	#
	df_doado_devol = df_ctosaluguel_trade[(df_ctosaluguel_trade['quantidade']<0) & (df_ctosaluguel_trade['ID']=='Devolucao')]
	df_doado_devol = df_doado_devol.groupby('codigo', as_index=False).agg({'quantidade': sum})
	df_doado_devol['quantidade'] = -df_doado_devol['quantidade']
	df_tomado_devol = df_ctosaluguel_trade[(df_ctosaluguel_trade['quantidade']>0) & (df_ctosaluguel_trade['ID']=='Devolucao')]
	df_tomado_devol = df_tomado_devol.groupby('codigo', as_index=False).agg({'quantidade': sum})
	...
	df_ctosaluguel['vencimento'] = pd.to_datetime(df_ctosaluguel['vencimento']).dt.strftime('%Y-%m-%d')
	df_vcto_0 = df_ctosaluguel[df_ctosaluguel['vencimento']==dt.strftime('%Y-%m-%d')]
	df_vcto_1 = df_ctosaluguel.loc[df_ctosaluguel['vencimento']==dt_next_1.strftime('%Y-%m-%d')]
	df_vcto_2 = df_ctosaluguel[df_ctosaluguel['vencimento']==dt_next_2.strftime('%Y-%m-%d')]
	df_vcto_3 = df_ctosaluguel[df_ctosaluguel['vencimento']==dt_next_3.strftime('%Y-%m-%d')]
	df_vcto_4 = df_ctosaluguel[df_ctosaluguel['vencimento']==dt_next_4.strftime('%Y-%m-%d')]
	...
	for c in set(df_recall['contrato']):
		for i in range(1, len(df_recall[df_recall['contrato'] == c])):
			...
			df_recall.loc[(df_recall['contrato'] == c) & (df_recall['data'] == d), 'recallD2'] = df_recall.loc[df_recall['contrato'] == c, 'recallD3'].iloc[i-1]
			df_recall.loc[(df_recall['contrato'] == c) & (df_recall['data'] == d), 'recallD1'] = df_recall.loc[df_recall['contrato'] == c, 'recallD2'].iloc[i-1]
			df_recall.loc[(df_recall['contrato'] == c) & (df_recall['data'] == d), 'PendRecallD3'] = df_recall.loc[df_recall['contrato'] == c, 'recallD3'].iloc[i]
			
			df_recall.loc[df_recall['contrato'] == c, 'DevolDia'].iloc[i] = df_recall.loc[df_recall['contrato'] == c, 'qtde'].iloc[i] -                                                             df_recall.loc[df_recall['contrato'] == c, 'qtde'].iloc[i-1]
			
			# pend recall d1
			df_recall.loc[(df_recall['contrato'] == c) & (df_recall['data'] == d), 'PendRecallD1'] = \
				max(df_recall.loc[df_recall['contrato'] == c, 'PendRecallD2'].iloc[i-1] \
				- df_recall.loc[df_recall['contrato'] == c, 'DevolDia'].iloc[i] \
				- df_recall.loc[df_recall['contrato'] == c, 'PendRecallD1'].iloc[i-1], 0)
			# Pend recall d2
			df_recall.loc[(df_recall['contrato'] == c) & (df_recall['data'] == d), 'PendRecallD2'] = \
				max(df_recall.loc[df_recall['contrato'] == c, 'recallD2'].iloc[i] \
				+ df_recall.loc[df_recall['contrato'] == c, 'PendRecallD2'].iloc[i-1] \
				- df_recall.loc[df_recall['contrato'] == c, 'DevolDia'].iloc[i] \
				- df_recall.loc[df_recall['contrato'] == c, 'PendRecallD1'].iloc[i], 0)

	df_recall_last = df_recall[df_recall['data']==dt]
	df_recall_g = df_recall_last.groupby('codigo', as_index = False).agg({'PendRecallD1':sum,'PendRecallD2':sum,'PendRecallD3':sum })
	...
	df=df.merge(df_recall_g[['codigo','PendRecallD1','PendRecallD2','PendRecallD3']], on='codigo', how='left')
	...
	df['to_borrow_2'] = np.minimum(0,df['custodia_2']-df['to_borrow_0']-df['to_borrow_1'])
	df['to_borrow_2'].fillna(0,inplace=True)
	df['to_borrow_3'] = np.minimum(0,df['custodia_3']-df['to_borrow_0']-df['to_borrow_1']-df['to_borrow_2'])
	...
	## Obs
	df['custodia_exaluguel'] = df['custodia_aux'] - df['net_alugado'] + np.minimum.reduce([df['mov_0'], df['mov_0']+df['mov_1'], df['mov_0']+df['mov_1']+df['mov_2']])


	df['devol_tomador']=np.minimum(-np.minimum(df['custodia_exaluguel'],0)-df['pos_tomada'],0)
	df['devol_doador']=np.maximum(-np.maximum(df['custodia_exaluguel'],0)-df['pos_doada']-df['PendRecallD1']-df['PendRecallD2']-df['PendRecallD3'],0)
	df['devol_tomador'].fillna(0,inplace=True)
	df['devol_doador'].fillna(0,inplace=True)

	df['to_lend Dia agg']= np.maximum(0,np.minimum(np.minimum(df['custodia_0'],df['custodia_1']),np.minimum(df['custodia_2'],df['custodia_3'])))
	df["to_lend"]=df.apply(lambda row: 0 if (row['custodia_exaluguel']+ row['pos_doada']<0) else row['custodia_exaluguel']+ row['pos_doada']  if row['custodia_exaluguel'] > 0 else 0 , axis=1)
...
def map(df):
	return df[['codigo','position','to_lend','to_lend Dia agg','to_lend Janela','taxa']]
